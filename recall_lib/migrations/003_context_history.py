...
def upgrade(conn):
    ...
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_history_version ON context_history(project_id, version)"
    )
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_history_key ON context_history(project_id, category, key)"
    )
